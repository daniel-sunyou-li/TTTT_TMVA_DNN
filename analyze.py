...
from ROOT import TH1D, TH2D, rootTree, TFile
from array import array
import varsList

lumiStr = str(weights.targetLumi / 1000.).replace(".","p") + "fb" # 1/fb
cutStr = varsList.cutStr

def analyze( rootTree, process, flv, doAllSys, doPDF, iPlot, plotDetails, catStr, year ):
...
# 'iPlot' is the variable tag
	...
	plot2D = False if len(plotDetails) < 3 else True
	if plot2D:
    ...
    if "tau32nm1" in iPlot.lowre(): cutStr += " && ( theJetAK8SoftDropCorr_JetSubCalc_PtOrdered > 105 && theJetAK8SoftDropCorr_JetSubCalc_PtOrdered < 220 )"
    ...
    else: nhottCut += " && NJetsTtagged == {}".format(nhott)
	...
	nWtagCut = ""
	if "p" in nWtagCut += " && NJetsWtagged >= {}".format(nWtag[:-1])
	...
	# declare histograms
	hists = {}
	if isPlot2D: hists["{}_{}_{}_{}{}".format(iPlot,lumiStr,catStr,process,flv)] = TH2D(
        iPlot+syst+ud+"_"+lumiStr+"_"+catStr+"_"+process+flv,
        yAxisLabel+xAxisLabel,
        len(ybins)-1,
        ybins,len(xbins)-1,
        xbins
    )
    else: hists["{}_{}_{}_{}{}".format(iPlot,lumiStr,catStr,process,flv)] = TH1D(
        iPlot+syst+ud+"_"+lumiStr+"_"+catStr+"_"+process+flv,
        xAxisLabel,
        len(xbins)-1,
        xbins
    ...
    for i in range(100):
        if isPlot2D: hists[iPlot+"pdf"+str(i)+"_"+lumiStr+"_"+catStr+"_"+process+flv] = TH2D(
